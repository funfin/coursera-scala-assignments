...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val pAcc = if(p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, pAcc))
  }
  ...
  override def mostRetweetedAcc(acc: Tweet): Tweet = {
    val rAcc = if(elem.retweets>acc.retweets) elem else acc
    left.mostRetweetedAcc(right.mostRetweetedAcc(rAcc))
  }
